...
const  App = ({authLogic}) => {
  console.log('App 호출')
  //상수값 -> axios(nodeJS,React-모듈화), fetch(바닐라-브라우저 지원) 공통점은 둘 다 비동기 처리방식임
  const [items, setItems] = useState([
    {id:1, name:"벤치프레스", count:0},
    {id:2, name:"랫풀다운", count:0},
    {id:3, name:"풀업", count:0}
  ]);
  //@param1-콜백함수임-객체
  //@param2-의존성배열-Dependency Array
  //:의존성 배열이 비어있으면 최초 App 컴포넌트가 렌더링(What?, Where) 될 때 딱 한 번만 실행됨
  //재렌더링이 되는 대상은 *return안에 있는 코드들이다.
  //만일 items가 변경되면 그 때는 재렌더링 일어남 (state가 변경되기 때문)
  useEffect(()=>{
  ...
  const handleAdd = (name) => {
    ...
    //세번째 파라미터는 0으로 초기화
    //스프레드 연산자를 활용하여 기존 배열에 한 개의 객체를 추가하는코드
    const workouts = [...items,{id:Date.now(),name,count:0}]
    //상태 훅에 반영 - 스프레드연산자를 활용하여 새로운 주소번지가 채번되도록 처리해야함
