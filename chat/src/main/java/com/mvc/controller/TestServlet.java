...
Logger logger = Logger.getLogger(TestServlet.class);
  ...
  throws ServletException, IOException
  {
	...
	// 담당하는 클래스에 연결(FrontController.java 설계 - 각 업무별 XXXController 필요함)을 해야 함 ]
	// 페이지 처리는 JSP에게 맡김
	...
	// 요청객체와 응답객체 전달되어야 한다
	// 요청 객체 무엇을 누릴 수 있나?
	// 사용자가 입력한 값을 듣기 위해 필요하다. 누가? getParameter가
	// request.getParameter("XXX"); mem_id, mem_name, mem_address 
	// HttpSession session = request.getSession();//세션객체생성
	// http 프로토콜이 비상태 프로토콜이므로 상태값을 관리하는 별도의 매커니즘 필요
	// 쿠키와 세션 (시간을 지배한다 *중요) <- List, Map <- 객체 배열 <- 배열 <- 원시형 타입 (변수)
	// response
	...
	// 그런데 마치 계속 유지하고 있는 것처럼 보여져야 함 -> 그러니까 쿠키나 세션에 담아둔다.
	// -> 왜냐하면 비상태 프로콜이니까!!
	// 쿠키 - 문자열 , 객체는 못 받음
	// 세션 - 캐시메모리, - 객체 받음
	// 아래 코드를 만나기 전까지는 http://localhost:9000/test/test.do
	// 를 보여주다가 아래 문자를 만나면 그 떄 http://localhost:9000/test/getMemberList.jsp
	// 테스트 시나리오
